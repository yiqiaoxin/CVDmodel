                         ...
                         mainPanel(
                           tabsetPanel(
                             ...
                             tabPanel("2. Base case results",
                                      br(),
                                      p("The base case analysis results are shown in the colored tiles below. The calculation may take up to 10 seconds."),
  ...
  server <- function(input, output, session) {
    ...
    output$intvqaly <- renderValueBox({
      valueBox(round(baseresult()$base_intv_allQALY -input$age,2),
               paste("Remaining quality-adjusted life-years, including", 
                     round(baseresult()$statin_disu,2), "disutility from taking statin daily"
               ), icon = icon("laugh-beam"), color = "yellow")
    })
    ...
    output$intvcost <- renderValueBox({
      valueBox(format(baseresult()$base_intv_allCost, big.mark=",", big.interval=3L,
               ...
               paste("Health care costs (GBP), including", 
                     format(baseresult()$statincost, big.mark=",", big.interval=3L,
                            digits=0, scientific=F), "(GBP) statin cost"
                     ) , 
               icon = icon("coins"), color = "light-blue")
    ...
    psa <-  eventReactive(input$runpsa, {
      ...
      progress$inc(0.1, detail="Completed all draws. Populating draws into the model...")
      ...
      for (i in 1:nrep) {
        ...
        if (i==nrep/4) {
          progress$inc(0.3, detail="Populating all draws into the model: 25% completed now ...")
        }

        if (i==nrep/2) {
          progress$inc(0.2, detail="Populating all draws into the model: 50% completed now ...")
        }

        if (i==nrep/4*3) {
          progress$inc(0.2, detail="Populating all draws into the model: 75% completed now ...")
        }
      ...
      progress$inc(0.2, detail="Completed. Rendering results now.")
