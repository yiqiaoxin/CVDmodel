    setwd("C:/Users/Yiqiao/OneDrive - University of Glasgow/Ongoing project/Converting CVD model to R/Final/Codes") #set working directory
    getwd()
    library(dplyr)
    library(tidyr)
    ...
    ### 0.1 user input parameters
      ...
      CPD <- 50                                                                               # No. of cigarette per day (continuous)
      SBP <- 160                                                                              # Systolic blood pressure
      TC <- 7                                                                                 # Total cholestorol 
      HDL <- 1
      sex <-1                                                                                 # male. sex <-2 female
      
      tm <- 115                                                                               # As consistent with the Excel model - time_horizon set as 115 cycles. Alternatively, if setting tm to be bounded by age 100: tm <- 100 - age
      disc <-0                                                                                # disc <-0 for undiscounted
      
      
    
    ### 0.2 load model parameters from survival analysis, cost analysis and utility. 
      source("CVD_data.R",local = TRUE)                                                       # This script contains two options (EXCEL or script) of loading data. If run the full script, data are loaded from EXCEL first and then the objects are re-written by the script. But they are the same anyway.
    ...
    ### 0.3 additional adjustment for the model
      calibration_f1 <- 0.96
      multi<-0.99
      
      first_event_coef[9,]<-first_event_coef[9,]*multi                                        # Constant
      first_event_coef[7:8,4] <- 0                                                            # coefficient adjustment for coefficient for TC and HDL for nonCVDdeath first event
  



    model <- function(age,SIMD,Diabetes,FH,CPD,SBP,TC,HDL,sex,first_event_coef,post_event_coef,disc, tm) {                               # Put the model as a function to run easily for PSA. Parameters in the model are varied in PSA except for disc.
    ...
    ### 2.1 Cost setup by sex                                                                 # 't1, t2 all 1-100, whereas the cycles are 1-115. At the later cycles, t1, t2 are assumed to be 0 in the excel model.'
    ...
    'if the dcat1 is calculated from cycle 3:  awaiting Ciarans reply
    c_preCHD_und<-sum(riskf_v2*coef_c1[3:5, 1]) + coef_c1[6,1]+coef_c1[1,1]*coef_c2[3:100,2]+coef_c1[2,1]*coef_c2[3:100,5]'
    ...
    ## Create a parameter matrix for t1, t2, and age (thse parameters change vertically or horizontally in the matrix)
    ...
    ## Cost conditional on the undiscounted survival at each year after the event
    ...
    'again awaiting for Ciarans reply. used cycle 3 rather than 1 for ti1 and ti2 values.'
    c_fatalcvd<-sum(riskf_v2*coef_c1[3:5,5])+coef_c1[6,5]+coef_c1[1,5]*coef_ct[,2]+coef_c1[2,5]*coef_ct[,5]
    c_fatalnoncvd<-sum(riskf_v2*coef_c1[3:5,6])+coef_c1[6,6]+coef_c1[1,6]*coef_ct[,2]+coef_c1[2,6]*coef_ct[,5]
    ...
    mc <-cbind(c_chd, c_cbvd, c_fatalcvd,c_fatalnoncvd)
    
    
    ### 2.6 KMSA - weighted cost (use probability for each event at each year (equation 1))
    cw<- prob_event_yr*mc
    
    ### 2.7 Results ---------->>  Total cost                                                  # This should be copmared to 'male parameters' - B5
    ...
    'Angina_Dec <-	0.0891, Irreg__Dec <-	0.0499  # Not used?'
    ...
    dec <- c(MI__Dec,Stroke__Dec,Other__Dec,PAD__Dec,Other__Dec)                              # Order as for secondary event coefficent sets order
    ...
    ### 3.8  KMSA - weighted QALY                                                             # use probability for each event at each year (equation 1), same for LY and COST)
    ...
    #<<<<>>>># Model output
    list <- list("LY"= sum(Total_LY)+age, "Cost" = sum(cw), "QALY" = sum(QALYw)+age)
    return(list)
    }
    ...
    model(age,SIMD,Diabetes,FH,CPD,SBP,TC,HDL,sex,first_event_coef,post_event_coef, 0, tm)        # undiscounted
    base <- model(age,SIMD,Diabetes,FH,CPD,SBP,TC,HDL,sex,first_event_coef,post_event_coef, disc, tm)   # discounted
    ...
    psa_result <- matrix(nrow = nrep, ncol = 3)
    colnames(psa_result) <- c("LY", "Cost", "QALY")
    for (i in 1:nrep) {
      ...
      first_event_coef[7:8,4] <- 0                                                           # adjustment of the coefficient to be 0 due to the unrealistic regression results. See excel for explanations
      psa <- model(age,SIMD,Diabetes,FH,CPD,SBP,TC,HDL,sex,first_event_coef,post_event_coef, disc, tm)                                   # can set discount rate to be anything here.
      psa_result[i,1] <- psa$LY
      psa_result[i,2] <- psa$Cost
      psa_result[i,3] <- psa$QALY
      }
